...
namespace run
{
	...
	int main()
	{
		...
		{
			poly p(b[i]);
			const int P = a[i] * is % MOD;
			for (int j = 0, pw = 1; j < b[i]; j++, pw = 1ll * pw * P % MOD)
				p[j] = 1ll * pw * Invs[j] % MOD;
			DEP(j, S, 0)
			{
				f[j] = f[j] * (poly(1, 0) - p);
...
//		REP(j, 0, S) output(f[j]);
		i64 ans = 0;
		REP(d, 0, S - 1)
		{
			i64 xs = inv(MOD + 1 - d * is % MOD), I = xs;
			REP(l, 0, deg(f[d]))
			{
				if (l) xs = I * xs % MOD * l % MOD;
				ans = (ans + xs * f[d][l]) % MOD;
			}
		}
