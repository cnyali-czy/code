...
signed main()
{
	...
	REP(Case, 1, q)
	{
		...
		if (!isfrm[t]) ans[Case] = min(d1[n], min(d1[u[t]] + x + dn[v[t]], d1[v[t]] + x + dn[u[t]]));
		else if (x <= v[t]) ans[Case] = min(d1[u[t]] + x + dn[v[t]], d1[v[t]] + x + dn[u[t]]);
		else
		{
			ans[Case] = d1[n] + x - w[t << 1];

			int p = siz[u[t]] < siz[v[t]] ? u[t] : v[t];
			...
			REP(i, 1, m) if (i ^ t)
			{
				...
				if (!in(v[i], p) && in(u[i], p))
					res = min(res, d1[v[i]] + w[i << 1] + dn[u[i]]);
			}
			*/
