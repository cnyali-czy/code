...
namespace real
{
	int n, q, type, fa[maxn], v[maxn], a[maxn], b[maxn];

	int siz[maxn], top[maxn], dfn[maxn], dfs_clock, hvy[maxn];
	vector <int> G[maxn];
	void dfs(int x)
	{
		siz[x] = 1;
	...
	void dfs2(int x, int y)
	{
		...
		if (!hvy[x]) return;
		dfs2(hvy[x], y);
		for (int y : G[x]) if (y ^ hvy[x]) dfs2(y, y);
	}
...
#define rs p << 1 | 1
	...
	int main()
	{
		n = read<int>();q = read<int>();type = read<int>();
		...
		while (q--)
		{
			int x = read<int>(), c = read<int>();
			if (type) x ^= lastans, c ^= lastans;
			int X = x;
			while (x)
			{
				b[x] = 1ll * c * b[x] % MOD;
