...
namespace run
{
	...
	int main()
	{
		...
		i64 f0 = f[1], if0 = inv(f0);
		REP(i, 1, n) f[i] = f[i] * if0 % MOD;
		b0[0] = 1;REP(i, 1, n) b0[i] = b0[i - 1] * f0 % MOD;
		poly g = fhn(f);

		bin[0].resize(n + 1);bin[0][0] = 1;
		bin[1] = f;
		REP(i, 2, n)
		{
			bin[i] = bin[i - 1] * f;
		...
		REP(i, 1, n)
		{
			pw[i] = power_pow(i, i - 1);
			i64 ans = 0;
			REP(j, 1, i) (ans += 1ll * pw[j] * Invs[j] % MOD * b0[j] % MOD * bin[j][i]) %= MOD;
			printf("%lld%c", ans * fac[i] % MOD, i == end_i ? '\n' : ' ' );
		}
		return 0;
	}
	int qaq_main()
	{
		n = read<int>();prepare(n);
		f.resize(n);
		REP(i, 0, n - 1) f[i] = 1ll * read<int>() * Invs[i] % MOD;
		i64 f0 = f[0], if0 = inv(f0);
		REP(i, 0, n - 1) f[i] = f[i] * if0 % MOD;
		b0[0] = 1;REP(i, 1, n) b0[i] = 1ll * b0[i - 1] * f0 % MOD;
		...
		REP(i, 1, n)
		{
			pw[i] = power_pow(i, i - 1);
			i64 ans = 0;
			REP(j, 1, i) (ans += 1ll * pw[j] * Invs[j] % MOD * b0[j] % MOD * bin[j][i - j]) %= MOD;
			printf("%lld%c", ans * fac[i] % MOD, i == end_i ? '\n' : ' ' );
		}
		return 0;
	}
