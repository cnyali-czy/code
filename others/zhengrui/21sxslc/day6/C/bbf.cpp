...
namespace real
{
	...
	void getf()
	{
		...
		REP(i, 1, n)
		{
			dp[i][k + 1][i] = 1;
			DEP(j, k, 1)
				REP(l, 0, i) REP(x, 0, l)//i - l)
				{
					i64 xs = calc0(i - l, a[j + 1] - a[j] - 1);
					if ((l - x) & 1)
						(xs *= calc1(i - l)) %= MOD;
					else
						(xs *= calc0(i - l, 1)) %= MOD;
					dp[i][j][l - x] = (dp[i][j][l - x] + dp[i][j + 1][l] * C(l, x) % MOD * xs) % MOD;
				}
		}
		*/
		REP(i, 0, n)
		{
//			X[i] = dp[i][1][0] * calc0(i, a[1]) % MOD;
			i64 res = (i == 0);
			DEP(j, k, (i & 1) ? k : 1)
			{
				i64 forone = 0, forzer = 0;
				forone = pre[j - 1];
				forzer = here[j];
				REP(l, 0, i - 1) if (l % 2 == 0)
					(res += C(i, l) * power_pow(forone, l) % MOD * power_pow(forzer, i - l - 1)) %= MOD;
			}
			X[i] = res;
//			if (res != X[i]) cerr << res << ' ' << X[i] << endl;
		}
	}
	...
	signed main()
	{
		...
		REP(i, 0, n)
		{
//			(ans += 1ll * X[i] * G[n] % MOD * Invs[i]) %= MOD;
