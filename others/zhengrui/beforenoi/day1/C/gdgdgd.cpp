...
#define rson rs, mid + 1, r
	...
	inline double h(double l, double r)
	{
...
#undef mid
	void work(i64 l, i64 r, int fl, int fr, double sl, double sr)
	{
		if (sqrt(r) - sqrt(l) < eps) return;
		if (fl == fr)
		{
			ans += fl * (sqrt(r) - sqrt(l));
			return;
		}
		i64 mid = (l + r) / 2;
		int fm = f(mid);
		work(l, mid, fl, fm, sl, sqrt(mid));
		work(mid + 1, r, f(mid + 1), fr, sqrt(mid + 1), sr);
	}
	int main()
	{
		...
		double s1 = 1, slim = sqrt(lim);
		for (o = 1; o <= n; o++)
		{
			ans = 0;
			work(1, lim, f(1), f(lim), s1, slim);
			printf("%.10lf\n", ans);
		}
		return 0;
	}
