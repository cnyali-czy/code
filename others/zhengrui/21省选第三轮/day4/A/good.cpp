...
int main()
{
	...
	*/
	...
	bpoly B(n + 1, poly(n + 1, 0));REP(i, 1, n) REP(j, 1, n) B[i][j] = Bin[i][j];
	...
	*/
	for (int b = n; b; )
	{
		Res = fix(Res * cur, n, n) + Res;
		cur = sqr(cur);
		b >>= 1;
	REP(i, 0, n) REP(j, 0, n) printf("%d%c", Res[i][j], j == end_j ? '\n' : ' ' );
	}
	Res = fix(Res * B, n, n);
	REP(i, 1, n) REP(j, 1, n) f[i][j] = Res[i][j];
//	REP(i, 1, n) REP(j, 1, n) printf("%d%c", Res[i][j], j == end_j ? '\n' : ' ' );
