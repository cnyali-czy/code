...
const double eps = 1e-9, H = 1e10, C = 1e8;
...
#endif
	long long ta, tb;
	scanf("%d%lld%lld", &n, &ta, &tb);a = ta;b = tb;
	if (n == 1) return 0 * puts("1");
	b = 1 / b;a = a * b;
	p[0] = vec(a *= H, b *= H);
	REP(i, 1, n)
	{
		long long tx, ty;scanf("%lld%lld", &tx, &ty);
		double x = tx, y = ty;
		x = 1 / x / 2, y = y * x;
		p[i] = vec(x * H, y * H, i);
		if (p[i].x < p[0].x || ABS(p[i].x - p[0].x) < eps && p[i].y < p[0].y) swap(p[0], p[i]);
	}
	auto B = p[0];
	REP(i, 0, n) p[i] = p[i] - B;
	sort(p + 1, p + n + 1, cmp);
...
//	REP(i, 0, n) printf("%.2LF %.2LF %d\n", stk[i].x, stk[i].y, stk[i].id);
	...
	{
		stk[i] = stk[i] + B;
		if (ABS(stk[i].x) > C || ABS(stk[i].y) > C) cout << stk[i].id << ' ' ;
	}
	return 0;
}
