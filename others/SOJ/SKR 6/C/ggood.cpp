...
{
	static ui64 c[maxn][maxn];
	static Matrix res;
	REP(i, 1, n) REP(k, 1, n) if (A.a[i][k]) REP(j, 1, n) if (B.a[k][j])
	{
		c[i][j] += (ui64)A.a[i][k] * B.a[k][j];
		if (c[i][j] >= LIM) c[i][j] %= MOD;
	}
	REP(i, 1, n) REP(j, 1, n) res.a[i][j] = c[i][j], c[i][j] = 0;
	return res;
}
...
struct Graph
{
	...
	poly solve()
	{
		...
		static ui64 t[maxn][maxn], old[maxn][maxn], ok[maxn][maxn];
		p1[0] = b;
		REP(i, 1, n) REP(j, 1, n) old[i][j] = p1[0].a[i][j];
		REP(I, 1, w)
		{
			REP(i, 1, n) REP(k, 1, n) if (A.a[i][k])
				REP(j, 1, n) t[i][j] += old[k][j];
			REP(i, 1, n) REP(j, 1, n) old[i][j] = p1[I].a[i][j] = t[i][j] % MOD, t[i][j] = 0;
		}
		cerr<< clock() * 1. / CLOCKS_PER_SEC << endl;
		p2[0] = b;
		REP(i, 1, n) REP(j, 1, n) ok[i][j] = p1[w].a[i][j], old[i][j] = (i == j);
		REP(I, 1, (n + w - 1) / w)
		{
			REP(i, 1, n) REP(k, 1, n) if (ok[i][k])
			{
				register ui64 x = ok[i][k];
				REP(j, 1, n) 
				{
					t[i][j] += x * old[k][j];
			...
			REP(i, 1, n) REP(j, 1, n) old[i][j] = p2[I].a[i][j] = t[i][j] % MOD, t[i][j] = 0;
			//p1[I] = p1[I - 1] * A
			//			p2[I] = p2[I - 1] * p1[w];
		}
		...
		REP(i, 0, n - 1)
		{
			if (pre[i] != qaq[i]) cerr << pre[i] << ' ' << qaq[i] << endl;
		}
		*/
		REP(i, 0, n - 1) pre[i] = qaq[i];
		poly c(1, 1);
		REP(i, 0, L)
		{
			i64 res = 0;
