...
#define file(FILE_NAME) freopen(FILE_NAME".in", "r", stdin), freopen("old.out", "w", stdout)
...
struct Graph
{
	...
	poly solve()
	{
		...
		REP(i, 1, n) REP(j, 1, n) printf("%d%c", A.a[i][j], j == end_j ? '\n' : ' ' );
		for (auto &i : f) i = (i + MOD) % MOD;
		printf("tzdxs: ");output(f);puts("");
		/*
		   puts("!!!");
		   REP(i, 1, n) REP(j, 1, n) printf("%d%c", A.a[i][j], j == end_j ? '\n' : ' ' );
		   output(f);
