...
int bg[maxn], ne[maxq << 1], to[maxq << 1], w[maxq << 1], e;
...
namespace bf
{
	...
	{
		bfs();if (!ok[x]) return void(puts("-1"));
		bfs(y);puts(ok[x] ? "0" : "1");
	}
	Decimal dis[maxn], inf;
	struct node
	{
		int x;Decimal d;
		node() {}
		node(int x, Decimal d) : x(x), d(d) {}
	...
	void getdis()
	{
		...
		REP(i, 1, m) Q.emplace(i, dis[i] = 1);
		REP(i, m + 1, n) dis[i] = inf;
		while (Q.size())
		{
			auto Node = Q.top();Q.pop();
			int x = Node.x;auto d = Node.d;
			if (d > dis[x]) continue;
			for (int i = bg[x]; i; i = ne[i])
			{
				int y = to[i], z = w[i];
				if (dis[y] < inf && dis[z] > d + dis[y])
					Q.emplace(z, dis[z] = d + dis[y]);
			}
		}
	}
	void work2(int T)
	{
		...
		bfs();
		if (lst != cur)	getdis();
		if (!ok[T]) return void(puts("-1"));
		if (dis[T] < inf) cout << dis[T] % 998244353 << '\n';
		else puts("-1");
	}
	void work()
	{
		cur = -1;
		inf = 1;
		REP(i, 1, 1010) inf = inf * 2;
		REP(Case, 1, q)
		{
			int opt = read<int>(), x = read<int>();
			if (opt == 1) work1(x, read<int>());
			else if (opt == 2) work2(x);
			else
			{
				int y = read<int>(), z = read<int>();
				add(x, y, z);add(y, x, z);
				lst = Case;
			}
		}
