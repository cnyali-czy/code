...
namespace run
{
	int fac[maxn], Invs[maxn], h[maxn], f[maxn];
	i64 sgn(int n)
	{
		return (n & 1) ? MOD - 1 : 1;
	}
	...
	int main()
	{
		...
		h[0] = 1;
		REP(i, 1, n) h[i] = (h[i - 1] + sgn(i) * Invs[i]) % MOD;
		REP(i, 1, n) h[i] = 1ll * fac[i] * h[i] % MOD;
		REP(i,1,n)h[i]=fac[i-1];
		i64 res = 0;
		REP(i, 2, n)
		{
			int ccc = (MOD + 1 - 1ll * h[i] * Invs[i] % MOD) % MOD;
			f[i] = inv(ccc) * (res + i) % MOD;
			(res += 1ll * h[i] * f[i] % MOD * Invs[i]) %= MOD;
		}
//		REP(i, 1, n) printf("%d%c", f[i], i == end_i ? '\n' : ' ');
//		REP(i, 1, n) printf("%d%c", h[i], i == end_i ? '\n' : ' ');
		n = read<int>();
		REP(Case, 1, read<int>())
		{
			REP(i, 1, n) a[i] = read<int>(), vis[i] = 0;
			i64 ans = 0;
