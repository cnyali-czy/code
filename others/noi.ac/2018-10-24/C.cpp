...
{
	if (l == r) mul[p] = Max[p] = Min[p] = a[l], sum[p] = a[l] + (long long)sqrt(a[l]);
	else
	{
		build(lson);
		build(rson);
...
{
	if (l == pos && pos == r) mul[p] = Max[p] = Min[p] = val, sum[p] = val + (long long)sqrt(val);
	else
	{
...
int main()
{
	...
	cin >> n >> m;
	fac[0] = 1;Inv[0] = 1;
	REP(i, 1, m) fac[i] = fac[i - 1] * (long long) i % MOD, Inv[i] = inv(fac[i]), Sum[i] = (Sum[i-1] + i + (long long)sqrt(i)) % MOD;
