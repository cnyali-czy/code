...
const int prime_g = 3, MOD = 1004535809, inv2 = MOD + 1 >> 1;
...
int main()
{
	...
	REP(i, 0, lim)
	{
		f[i] = mul(C(n, s * i), C(m, i));
		f[i] = mul(f[i], mul(fac[s * i], bin));
		f[i] = mul(f[i], mul(fac[i], power_pow(m - i, n - s * i)));
		bin = mul(bin, Inv[s]);
	}
	REP(i, 0, lim)
		g[lim - i] = mul(i & 1 ? MOD - 1 : 1, Inv[i]);
#ifdef CraZYali
	REP(i, 0, lim) printf("%d%c", f[i], i == end_i ? '\n' : ' ' );
	REP(i, 0, lim) printf("%d%c", g[i], i == end_i ? '\n' : ' ' );
#endif
	int l = 1;
	while (l <= lim + lim) l <<= 1;
	NTT(f, l, 1);NTT(g, l, 1);
	...
	NTT(f, l, -1);
	int ans = 0;
	REP(i, 0, lim) inc(ans, mul(read<int>(), mul(f[lim + i], Inv[i])));
	cout << ans << '\n';
	return 0;
}
