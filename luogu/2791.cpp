...
const int orzjjs = 2e7 + 10;
int fac[orzjjs], Invs[orzjjs], s2[maxn];
...
int main()
{
	...
	init(max(L, N));
	S2 :: calc(L);
	REP(i, 0, L) s2[i] = 1ll * s2[i] * fac[i] % MOD;
	while (S--)
	{
		...
		i64 ans = 0;
		REP(j, 0, min(m, L))
			ans += s2[j] * C(m, j) % MOD * C(n - j, k - j) % MOD;
		printf("%d\n", ans % MOD * Invs[n] % MOD * fac[k] % MOD * fac[n - k] % MOD);
	}
	return 0;
}
