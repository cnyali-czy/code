...
poly MTT(poly f, poly g, const int MOD)
{
	...
	REP(i, 0, max(n, m))
	{
		...
		F[1][i] = Complex((double)a + d + b + c, (double)a - d + b - c);
		F[2][i] = Complex(b + d, b - d);
	}
	REP(i, 0, 2)
	{
		FFT(F[i], l, 1);
	...
	REP(i, 0, n + m)
	{
		i64 A = (i64)(F[0][i].a + .5) % MOD, B = (i64)(F[1][i].a + .5) % MOD, C = 0, D = (i64)(F[2][i].a + .5) % MOD;
		i64 tmp = (A * w2 + (B + C) * w + D) % MOD;
		f[i] = Complex((tmp + MOD) % MOD);
	}
	return f;
}
