...
using namespace std;
const int maxn = 27, maxk = 1e5 + 10, MOD = 1e9 + 7;
	...
	int det(int labda)
	{
		...
		REP(i, 1, n)
		{
			...
			REP(j, i + 1, n) if (a[j][i])
			{
				...
				REP(k, i, n) a[j][k] = (a[j][k] - a[i][k] * tmp) % MOD;
			}
		}
		return ans;
	}
...
namespace Gauss
{
	...
	void solve()
	{
		REP(i, 0, n)
		{
			...
			REP(j, i + 1, n) if (a[j][i])
			{
				const i64 tmp = 1ll * a[j][i] * Inv % MOD;
				REP(k, i, n + 1) a[j][k] = (a[j][k] - a[i][k] * tmp) % MOD;
			}
		}
		DEP(i, n, 0)
		{
			REP(j, i + 1, n) a[i][n + 1] = (a[i][n + 1] - 1ll * a[i][j] * g[j]) % MOD;
...
int main()
{
	...
	while (N)
	{
		if (N & 1)
		{
			...
			REP(i, 0, n + n) tmp[i] = 0;
			REP(i, 0, n) if (ans[i])
				REP(j, 0, n) tmp[i + j] = (tmp[i + j] + 1ll * ans[i] * base[j]) % MOD;
			REP(i, 0, n + n) ans[i] = tmp[i];
			//ans %= MOD
			DEP(i, n + n, n) if (ans[i])
			{
				const i64 tmp = 1ll * ans[i] * ig[n] % MOD;
				REP(j, 0, n) ans[i - j] = (ans[i - j] - tmp * g[n - j]) % MOD;
			}
		}
		//base *= base
		REP(i, 0, n + n) tmp[i] = 0;
		REP(i, 0, n) if (base[i])
			REP(j, 0, n) tmp[i + j] = (tmp[i + j] + 1ll * base[i] * base[j]) % MOD;
		REP(i, 0, n + n) base[i] = tmp[i];
		//base %= MOD
		DEP(i, n + n, n) if (base[i])
		{
			const i64 tmp = 1ll * base[i] * ig[n] % MOD;
			REP(j, 0, n) base[i - j] = (base[i - j] - tmp * g[n - j]) % MOD;
		}
		N >>= 1;
...
//	REP(i, 0, n) printf("%d%c", (ans[i] + MOD) % MOD, i == n ? '\n' : ' ');
	...
	REP(qaq, 0, n)
	{
		if (ans[qaq]) REP(i, 1, n) REP(j, 1, n) res[i][j] = (res[i][j] + 1ll * ans[qaq] * I[i][j]) % MOD;
		REP(i, 1, n) REP(j, 1, n) Mtmp[i][j] = 0;
		REP(i, 1, n)
			REP(k, 1, n) if (I[i][k])
			REP(j, 1, n) Mtmp[i][j] = (Mtmp[i][j] + 1ll * I[i][k] * M[k][j]) % MOD;
