...
inline poly operator * (poly a, poly b)
{
	...
	while ((1 << n) ^ len) n++;
	printf("shit : ");
	REP(i, 0, (1 << n) - 1) printf("%d%c", a[i], i == (1 << n) - 1? '\n' : ' ');
	REP(i, 0, (1 << n) - 1) printf("%d%c", b[i], i == (1 << n) - 1? '\n' : ' ');
	...
	REP(i, 0, (1 << n) - 1) printf("%d%c", c[i], i == (1 << n) - 1? '\n' : ' ');
	return c;
}
...
int main()
{
	...
	REP(Case, 1, read<int>())
	{
		...
		REP(i, 0, n) REP(j, 0, lim) printf("%d%c", mem[i][j], j == end_j ? '\n' : ' ' );
		poly F, G, H;
		G = H = mem[n];
		DEP(i, n - 1, 0)
		{
			g.clear();h.clear();g = h = poly(1 << n, 0);
			REP(j, 0, lim) if (j >> i & 1)
			{
				g[j ^ (1 << i)] = H[j];
				h[j ^ (1 << i)] = mem[i][j];
