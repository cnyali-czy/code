...
char str[M];
#define info pair <char, unsigned short>
#define tar first
#define mat second
...
void New(int id, int lft)
{
    ...
    REP(x, 0, AC :: cur) REP(i, 0, len[id] - 1)
        g[0][x][i + lent[id]] = make_pair(AC :: ch[x][str[i] - 'a'], AC :: s[AC :: ch[x][str[i] - 'a']]);
    REP(k, 1, 17) REP(x, 0, AC :: cur) REP(i, 0, len[id] - 1)
    {
        int y = g[k - 1][x][i + lent[id]].first;
