...
int main()
{
	...
	REP(k, 1, n - 1)
	{
		...
		REP(i, 1, n) REP(j, 1, i)
		{
			...
			{
				f[i][j] = (f[i][j] + 
				F[i - j][lstj] * C(i - 1, i - j - 1) % MOD * all(j) % MOD * power_pow(bin[lstj] - 1, j)
				)
				% MOD;
				g[i][j] = (g[i][j] + 
				G[i - j][lstj] * C(i - 1, i - j - 1) % MOD * all(j) % MOD * power_pow(bin[lstj] - 1, j)
				) % MOD;
				
			}
			g[i][j] = (g[i][j] + 1ll * k * f[i][j] % MOD * j) % MOD;
		}
		REP(j, 1, n) ans += g[n][j];
	}
