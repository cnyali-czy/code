...
//			REP(i, 0, 3)
			if (S >> 0 & 1) ard += L[0];
			else			ard += R[0], sgn = -sgn;
			if (S >> 1 & 1) ard += L[1];
			else			ard += R[1], sgn = -sgn;
			if (S >> 2 & 1) ard += L[2];
			else			ard += R[2], sgn = -sgn;
			if (S >> 3 & 1) ard += L[3];
			else			ard += R[3], sgn = -sgn;
			if (ard > s) continue;
...
int N;
pair <int, int> A[maxm * 2], A2[maxm * maxm * 4];
i64 A4x()
{
	i64 res = 0;
	...
	for (int i = N, k = 0; i >= 1; i--)
	{
		while (k < N && A2[i].first + A2[k + 1].first <= s)
		{
			...
			s1 += t;
			t *= A2[k].first;
			//			s4 = (s4 + 1ll * A2[k].second * A2[k].first) % MOD;
			s4 = (s4 + t) % MOD;
			t = t % MOD * A2[k].first;
			//			s3 = (s3 + 3ll * A2[k].second * A2[k].first % MOD * A2[k].first) % MOD;
			s3 = (s3 + 3ll * t) % MOD;
			t = t % MOD * A2[k].first;
			//			s2 = (s2 + 1ll * A2[k].second * A2[k].first % MOD * A2[k].first % MOD * A2[k].first) % MOD;
			s2 = (s2 + t) % MOD;
		}
		...
		i64 cur2 = 0;
		cur += (2 - 3 * b + b * b) % MOD * s1 % MOD * b % MOD;
		cur -= s2;
		cur += s3 * (b - 1) % MOD;
		cur += (-2 + 6 * b - 3 * b * b) % MOD * s4 % MOD;

		res = (res + cur/* % MOD */ * fi) % MOD;
	}
...
{
	i64 res = 0;
	REP(i, 1, m) REP(j, 1, m) REP(k, j, m)
	{
		int L[] = {2 * l[i], l[j], l[k]};
		int R[] = {2 * (r[i] + 1), r[j] + 1, r[k] + 1};
		if (L[0] + L[1] + L[2] > s) continue;
		i64 cur = 0;
		REP(S, 0, 7)
		{
			int sgn = 1, ard = 0;
...
i64 Ax2A2x()
{
	...
	REP(i, 1, N)
	{
		int a = A2[i].first, fa = A2[i].second;
		if (a > s) break;
		REP(j, 1, m + m)
		{
			int b = A[j].first * 2, fb = A[j].second;
			if (a + b > s) break;
			res += fa * fb * magic1(s - a - b);
		}
	}
	res %= MOD;
	if (res < 0) res += MOD;
...
namespace four
{
	...
	void work()
	{
		...
		sort(A2 + 1, A2 + 1 + N);
		static pair <int, int> tmpA2[maxm * maxm * 4];
		int tN = 0;
		REP(i, 1, N)
			if (A2[i].first == A2[i - 1].first) tmpA2[tN].second += A2[i].second;
			else tmpA2[++tN] = A2[i];
		swap(N, tN);swap(A2, tmpA2);
		i64 ans1, ans2, ans3, ans4, ans5;
...
//		ans = (A4x() - 6 * Ax2A2x() + 3 * A2x2() + 8 * Ax3Ax() - 6 * Ax4()) * inv24 % MOD;
		ans = (ans1 - 6 * ans2 + 3 * ans3 + 8 * ans4 - 6 * ans5) * inv24 % MOD;
		if (ans < 0) ans += MOD;
	}
}
