...
int invs[maxn], Invs[maxn], fac[maxn], last;
void prepare(int n)
{
	if (!last)
	{
...
{
	for (auto &i : f) i = 1ll * x * i % MOD;
	return f;
}
namespace run
{
	int n, m;
	void init(int n) {prepare(n);}
	int a[maxn];
	...
	poly rev(poly f) {reverse(f.begin(), f.end());return f;}
	poly fix(poly f, int n) {f.resize(n + 1);return f;}
	int main()
	{
		n = read<int>();m = read<int>();
		REP(i, 1, n) a[i] = read<int>() % MOD;
		init(m + 1);
		poly g(m + 1, 0);
		REP(i, 1, m + 1) g[i - 1] = Invs[i];
		g = Ln(g);
		poly h = cdq(1, n);
		poly f = fix(rev(Der(h)) * Inv(fix(rev(h), m)), m);
		REP(i, 0, m) f[i] = 1ll * g[i] * f[i] % MOD;
		f = Exp_log2(f);
		REP(i, 1, m) printf("%lld%c", 1ll * fac[i] * f[i] % MOD , i == end_i ? '\n' : ' ' );
		return 0;
	}
}
