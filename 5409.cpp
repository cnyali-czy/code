...
using namespace std;
const int MOD = 167772161, inv3 = (MOD + 1) / 3, inv2 = MOD + 1 >> 1;
...
int main()
{
	...
	int N = n - k;
	fac[0] = 1;
	REP(i, 1, n + 1) fac[i] = 1ll * i * fac[i - 1] % MOD;
	Invs[n + 1] = inv(fac[n + 1]);
	DEP(i, n, 0) Invs[i] = (i + 1ll) * Invs[i + 1] % MOD;
	poly f(N + 1);
	REP(i, 0, N) f[i] = Invs[i + 1];
	f = Ln(f);
	for (auto &i : f) i = 1ll * i * k % MOD;
	f = Exp_log2(f);
	REP(i, 0, n) printf("%d%c", i < k ? 0 : 1ll * f[i - k] * fac[i] % MOD * Invs[k] % MOD, i == end_i ? '\n' : ' ');
	return 0;
}
